...
def delete_camera(session_id: str):
...
def stream(session_id: str):
    ...
    surveillance.SESSION_ZONES[session_id] = {zone["id"]: zone["polygon"] for zone in camera.get("zones", [])}
    return StreamingResponse(surveillance.stream_frames(session_id, Path(camera["path"])), media_type="multipart/x-mixed-replace; boundary=frame")
...
def stop_stream(session_id: str):
    if not surveillance.get_camera(session_id):
        raise HTTPException(404, "upload session not found")
    return {"session_id": session_id, "stopped": surveillance.stop_stream(session_id), "message": "stream stopping; active events will be finalized"}


@router.get("/api/counts/{session_id}")
def counts(session_id: str):
    if session_id not in surveillance.SESSION_COUNTS and not surveillance.get_camera(session_id):
        raise HTTPException(404, "upload session not found")
...
def event_clip_meta(event_id: int):
    ...
    if not frames:
        raise HTTPException(404, "event clip is still recording or unavailable")
    # Uploaded and live events share the application's event store.
    return JSONResponse({"event_id": event_id, "fps": event_service.clip_fps(surveillance.DEMO_EVENTS, event_id), "frame_count": len(frames)})
